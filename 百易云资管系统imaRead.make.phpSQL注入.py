...
def poc(url):                       #用poc函数来输入url检测这个url的漏洞是否存在 
    ...
    hosts=url.replace("https://","")   #hosts是专门给头部使用的，不影响其他变量去除url中的协议，防止协议影响检测
    hosts=url.replace("http://","")   #去除url中的协议，防止协议影响检测
    print(hosts)
    headers={
    ...
    print(target)    #!!输出一行显示target，检测target这个url9路径是否正确
    #接下来写返回值判断是否能访问到
    try:
